...
Future<File> imageAddWaterMark(String imagePath, String textStr) async {
  ...
  var text = textStr;
  // 完整覆盖下的正方形面积
  double rectSize = double.parse(math.pow(dimension, 2).toString());
  // 根据面积与字符大小计算文本重复次数
  int textRepeating = ((rectSize / math.pow(40, 2) * 2) / (text.length + 8))
  ...
  canvas.translate(
      -pivotPoint
          .distanceTo(math.Point(0, double.parse(image.height.toString()))),
  ...
  var textPainter = TextPainter(
    text: TextSpan(
        ...
        style: TextStyle(
            fontSize: 40, color: Color.fromRGBO(0, 0, 0, .3), height: 2)),
    maxLines: null,
