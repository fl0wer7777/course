...
int main(void)
{
	...
	pthread_attr_setschedpolicy(&attr_a, policy);
	pthread_attr_setschedpolicy(&attr_b, policy);

	struct sched_param param_a, param_b;
	param_a.sched_priority = 1;
	param_b.sched_priority = 30;
	...
	pthread_attr_setschedparam(&attr_b, &param_b);
 	...
 	pthread_create(&th_a, &attr_a, producer, 0);
  	pthread_create(&th_b, &attr_b, consumer, 0);
